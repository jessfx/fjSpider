...
class anjukeSpider(Spider):
    ...
    def second_parse(self, response):
        ...
        dds = soup.find(
            'div', class_='houseInfoV2-detail clearfix').find_all('dd')
        ...
        item['price_averages'] = float(
            dds[9].get_text().strip().replace('元/m\xb2', '').strip())
        item['description'] = soup.find(
            'div', class_='houseInfoV2-item-desc js-house-explain').get_text().strip()
        item['house_support'] = soup.find(
            'div', class_='houseInfoV2-item-desc').get_text()
        item['contact'] = soup.find(
            'p', class_='brokercard-name').get_text().strip()
